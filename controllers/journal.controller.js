...
exports.getJournals = async (req, res) => {
  ...
  try {
    ...
    if (id) {
      const journal = await db("journals").select("*").where("id", id).first();
      return res.status(200).json(journal || {});
    }

    // Build base query
    let query = db("journals").select("*").where("user_id", req.user.id);
    ...
    if (search) {
      query = query.where(function () {
        this.where(
          db.raw("LOWER(title) LIKE ?", [`%${search.toLowerCase()}%`])
        ).orWhere(
...
exports.uploadJournalCloudinary = async (req, res) => {
  try {
    const { file } = req;
